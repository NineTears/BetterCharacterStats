			...
			if BCS_Tooltip:SetInventoryItem('player', slot) then
				...
				for line=1, BCS_Tooltip:NumLines() do
					...
					if left:GetText() then
						...
						_,_, value = strfind(left:GetText(), L["Equip: Improves your chance to get a critical strike with missile weapons by (%d)%%."])
						if value then
							BCScache["gear"].ranged_crit = BCScache["gear"].ranged_crit + tonumber(value)
						end
				...
				for talent=1, GetNumTalents(tab) do
					...
					for line=1, BCS_Tooltip:NumLines() do
						...
						if left:GetText() then
							...
							-- Force of Will
							_,_, value = strfind(left:GetText(), L["Increases your spell damage and the critical strike chance of your offensive spells by (%d+)%%"])
							if value and rank > 0 then
		...
		for talent=1, GetNumTalents(tab) do
			...
			for line=1, BCS_Tooltip:NumLines() do
				...
				if left:GetText() then
					...
					--enhancing totems
					_,_, value = strfind(left:GetText(), L["increases block amount by (%d+)%%"])
					if value and rank > 0 then
